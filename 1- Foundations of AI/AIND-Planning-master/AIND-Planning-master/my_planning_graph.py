...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for a in self.all_actions:
            current_a_node = PgNode_a(a)
            if current_a_node.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(current_a_node)
                for s in self.s_levels[level]:
                    if s in current_a_node.prenodes:
                        current_a_node.parents.add(s)  
                        s.children.add(current_a_node) 
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #returning True if one action negates an effect of the other, and False otherwise.
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # returning True if the effect of one action is the negation of a precondition of the other
        # for all effects +/- , all preconditions +/- for node_a1 & node_a2
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in goals:
            if goal not in self.s_levels[len(self.s_levels) - 1]:
                return float('inf')
            else:
                for current_level, s_level in enumerate(self.s_levels):
                    # if the current goal in the current level, then increase level_sum with the current level, then break because we take the first goal.
                    if goal in s_level:
